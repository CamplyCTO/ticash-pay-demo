...
/**
 * Agent create/reset password: enter phone → receive a one-time SMS code → set the
 * password. Serves BOTH a first-time agent (no password yet) and a forgot-password reset —
 * `resetPassword` sets the hash either way. This is the ONLY step that costs an SMS; every
 * subsequent login uses the password.
 */
export function ResetScreen() {
  ...
  return (
    <Screen scroll footer={<Button title={tr('auth.sendCode')} loading={loading} disabled={!phoneValid} onPress={onRequest} />}>
      <Text variant="title" style={{ marginTop: t.spacing(4) }}>{tr('auth.firstAccess')}</Text>
